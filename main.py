...
with open('E:\\Flask\\CWH flask\\flask-tut\config.json', 'r') as c:
...
def home():
    ...
    flash("This is a danger alert!", "danger")
...
def uploader():
    if('user' in session and session['user'] == params['admin_user']):
        if(request.method=='POST'):
            ...
            return "Uploaded Successfuuuulyyy"
...
def contact():
    if(request.method=='POST'):
        ...
        mail.send_message('New message from '+name, sender=email, recipients=[params['gmail_user']], 
            body=message+"\n"+phone)
        flash("Details submitted! We'll get back to you soon.", "success")
